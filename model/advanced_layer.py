...
class SimpleEmbLayer(nn.Module):
    ...
    def __init__(self, config, device = None):
        ...
        # self.hidden_size = 300 # raw attribute
        self.hidden_size = config.hidden_size # config.hidden_size use to test
...
class LinkSpecificGraphConvolution_v2(nn.Module):
    ...
    def __init__(self, in_features, out_features, link_num, bias=True):
        ...
        self.in_features = in_features
        self.out_features = out_features
        self.link_num = link_num # how many type of link in the graph 
        self.link_weight = nn.Parameter(torch.FloatTensor(link_num, out_features))
        self.reset_parameters()
    ...
    def forward(self, input, adj, hidden):
        node_num, h = input.size()
        input = input.view(node_num, h) # [node_num, hidden_state]
        adj = adj.view(node_num*node_num, -1) # [node_num * node_num, link_type] 
        support = torch.mm(self.link_weight, hidden.squeeze(0).unsqueeze(1)) # (link_num, 1), the weight different edge
        #print(adj, support)
        support = torch.mm(adj, support).squeeze(1) # (node_num * node_num, h)
        support = F.softmax(support.view(node_num, node_num), 1) # (node_num, node_num)
        #print(support[0, :])
        output = torch.mm(support, input) # the representation of each entity node
        return output
...
class DynamicGCN(nn.Module):
    ...
    def __init__(self, config, device = None):
        ...
        self.gcn = LinkSpecificGraphConvolution_v2(config.hidden_size, config.hidden_size, 3) # raw version
        # self.gcn = DeepResidualGCN(config, 3, 2)
