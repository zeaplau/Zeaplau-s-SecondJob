...
URL = 'http://43.154.164.160:8088/query' # my private server

def get_results(sparql_query):
    to_query = {
        'statement': sparql_query,
    }
    return requests.post(url=URL, data=to_query).json()
...
?e1 ?r wd:Q3127867}
"""     
        result = get_results(sparql_query=sparql_txt)
    ...
    def SQL_1hop(self, p, QUERY=None):
        ...
        for query_idx, query in enumerate(queries): #
            ...
            try:
                ...
                results = get_results(sparql_query=sparql_txt)
